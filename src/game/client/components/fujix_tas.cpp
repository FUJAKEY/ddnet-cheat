...
void CFujixTas::TickPhantom()
{
    ...
    {
        if((m_PhantomTick - m_StartTick) % m_PhantomStep == 0)
            m_PhantomPrevCore = m_PhantomCore;

        m_PhantomTick++;

        // Discard old inputs that we might have due to a rollback
        while(!m_PendingInputs.empty() && m_PendingInputs.front().m_Tick < m_PhantomTick)
            m_PendingInputs.pop_front();

        // Use the input for the current tick if available
        if(!m_PendingInputs.empty() && m_PendingInputs.front().m_Tick == m_PhantomTick)
        {
        ...
        {
            m_PhantomHistory.push_back({m_PhantomTick, m_PhantomCore, m_PhantomPrevCore, m_PhantomInput, m_PhantomFreezeTime});
            if(m_PhantomHistory.size() > 300) // Keep up to 6 seconds of history at 50 TPS
