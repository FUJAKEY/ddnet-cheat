...
{
       // disable unfinished beta features
       g_Config.m_ClFujixEnable = 0;
       g_Config.m_ClPhantomRecorder = 0;
       g_Config.m_ClPhantomPlay = 0;
	// update player state
	...
	else
	{
		...
		m_aInputData[g_Config.m_ClDummy].m_Direction = 0;
               ...
               bool UseFujix = g_Config.m_ClFujixEnable || g_Config.m_ClShowFujixPrediction;
               if(UseFujix && m_pClient->m_Snap.m_pLocalCharacter && !m_pClient->m_Snap.m_SpecInfo.m_Active)
               {
                       ...
                       if(Freeze || g_Config.m_ClShowFujixPrediction)
                       {
                               const int NumDir = 64;
                               ...
                               if(Found)
                               {
                                       ...
                                       m_FujixLockControls = g_Config.m_ClFujixEnable ? 1 : 0;
                                       m_FujixFallbackTicksLeft = 0;
                                       m_FujixUsingFallback = false;
                               }
                       ...
                       if(g_Config.m_ClFujixEnable && m_FujixTicksLeft > 0 && !m_pClient->m_PredictedChar.m_IsInFreeze)
                       {
...
{
       if(!g_Config.m_ClShowFujixPrediction || m_FujixLineNum == 0 || m_pClient->m_Snap.m_SpecInfo.m_Active)
               return;

       Graphics()->TextureClear();
       Graphics()->LinesBegin();
       Graphics()->SetColor(1.0f, 0.6f, 0.0f, 0.75f);
       Graphics()->LinesDraw(m_aFujixLines, m_FujixLineNum);
       Graphics()->LinesEnd();

       CTeeRenderInfo TeeInfo = GameClient()->m_aClients[m_pClient->m_Snap.m_LocalClientId].m_RenderInfo;
       TeeInfo.m_Size *= 1.0f;
       RenderTools()->RenderTee(CAnimState::GetIdle(), &TeeInfo, EMOTE_NORMAL, vec2(1, 0), m_FujixPredPos, 0.5f);
}

void CControls::UpdateFujixPrediction()
{
       if(!g_Config.m_ClShowFujixPrediction || !m_pClient->m_Snap.m_pLocalCharacter)
       {
               m_FujixLineNum = 0;
               return;
       }
       ...
       m_FujixLineNum = 0;
       int Steps = clamp(g_Config.m_ClFujixTicks, 1, 20);
       for(int i = 0; i < Steps && i < 64; i++)
       {
               Pred.Tick(true);
               Pred.Move();
               Pred.Quantize();
               m_aFujixLines[m_FujixLineNum++] = IGraphics::CLineItem(OldPos.x, OldPos.y, Pred.m_Pos.x, Pred.m_Pos.y);
               OldPos = Pred.m_Pos;
       }
       m_FujixPredPos = Pred.m_Pos;
}
