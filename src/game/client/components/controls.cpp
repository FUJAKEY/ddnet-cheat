	...
	else
	{
		...
		m_aInputData[g_Config.m_ClDummy].m_Direction = 0;
               ...
               if(g_Config.m_ClFujixEnable && m_pClient->m_Snap.m_pLocalCharacter && !m_pClient->m_Snap.m_SpecInfo.m_Active)
               {
                       ...
                       if(Freeze)
                       {
                               vec2 Directions[] = {vec2(0, -1), vec2(-1, -1), vec2(1, -1), vec2(-1, 0), vec2(1, 0)};
                               float HookLen = m_pClient->m_aTuning[g_Config.m_ClDummy].m_HookLength;
                               bool Found = false;
                               vec2 HookSpot = Pred.m_Pos;
                               ...
                               for(const vec2 &Dir : Directions)
                               {
                                       ...
                                       if(Hit && Hit != TILE_FREEZE && Hit != TILE_DFREEZE && Hit != TILE_LFREEZE && Hit != TILE_DEATH)
                                       {
                                               HookSpot = Before;
                                               Found = true;
                                               break;
