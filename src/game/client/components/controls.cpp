...
int CControls::SnapInput(int *pData)
{
	...
	else
	{
		...
		m_aInputData[g_Config.m_ClDummy].m_Direction = 0;
               ...
               {
                       auto IsFreeze = [&](vec2 Pos) {
                               int MapIndex = Collision()->GetPureMapIndex(Pos);
                       ...
                       if(Freeze)
                       {
                               ...
                               for(int k = 0; k < NumDir; k++)
                               {
                                       ...
                                       // simulate hooking a few ticks to see the outcome
                                       CCharacterCore Test = SafeCore;
                                       CNetObj_PlayerInput TestInput = PredInput;
                                       bool Fail = false;
                                       for(int t = 0; t < 5; t++)
                                       {
                                               vec2 d = normalize(Col - Test.m_Pos);
                                               TestInput.m_TargetX = (int)(d.x * GetMaxMouseDistance());
                                               TestInput.m_TargetY = (int)(d.y * GetMaxMouseDistance());
                                               TestInput.m_Hook = 1;
                                               Test.m_Input = TestInput;
                                               Test.Tick(true);
                                               Test.Move();
                                               Test.Quantize();
                                               if(IsFreeze(Test.m_Pos))
                                               {
                                                       Fail = true;
                                                       break;
                                               }
