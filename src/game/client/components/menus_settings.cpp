...
void CMenus::RenderSettingsFujix(CUIRect MainView)
{
       ...
       MainView.HSplitTop(20.0f, &Button, &MainView);
       static CLineInput s_PhantomNameInput;
       s_PhantomNameInput.SetBuffer(g_Config.m_ClPhantomName, sizeof(g_Config.m_ClPhantomName));
       Button.VSplitLeft(120.0f, &Label, &Button);
       Ui()->DoLabel(&Label, Localize("Phantom file"), 14.0f, TEXTALIGN_ML);
       Ui()->DoEditBox(&s_PhantomNameInput, &Button, 14.0f);

       MainView.HSplitTop(20.0f, &Button, &MainView);
       if(DoButton_CheckBox(&g_Config.m_ClPhantomPlay, "Play phantom (BETA)", g_Config.m_ClPhantomPlay, &Button))
               g_Config.m_ClPhantomPlay ^= 1;
}
