       ...
       if(!Dummy)
       {
               ...
               if(Size > 0)
               {
                     if(m_FujixTas.IsRecording())
                     {
                      ...
                      {
                              vec2 DummyPos = m_aClients[m_aLocalIds[!g_Config.m_ClDummy]].m_Predicted.m_Pos;
                              if(DummyPos.y > m_LocalCharacterPos.y && fabs(DummyPos.x - m_LocalCharacterPos.x) < 48 && DummyPos.y - m_LocalCharacterPos.y < 96)
                              {
                                      LocalInput.m_Hook = 1;
                      ...
                      if(g_Config.m_ClFujixAimbot && LocalInput.m_Hook)
                      {
                              ...
                              if(BestID >= 0)
                              {
                                      LocalInput.m_TargetX = (int)BestDir.x;
                                      LocalInput.m_TargetY = (int)BestDir.y;
                              }
                      }
