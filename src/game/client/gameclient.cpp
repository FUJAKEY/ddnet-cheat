...
static bool IsUnsafeHookTile(int Tile)
{
       return Tile == TILE_FREEZE || Tile == TILE_DFREEZE || Tile == TILE_LFREEZE || Tile == TILE_DEATH || Tile == TILE_NOHOOK;
}

bool CGameClient::FindSafeHookPos(const vec2 &Pos, vec2 &OutDir)
{
       float BestDist = -1.0f;
       const float HookLen = m_PredictedChar.m_Tuning.m_HookLength;
       for(int i = 0; i < 128; ++i)
       {
               float Angle = i * (2.0f * pi / 128.0f);
               vec2 Target = Pos + vec2(std::cos(Angle), std::sin(Angle)) * HookLen;
               vec2 Col, Before;
               int Hit = Collision()->IntersectLineTeleHook(Pos, Target, &Col, &Before);
               if(!Hit || Hit == TILE_NOHOOK || Hit == TILE_TELEINHOOK)
                       continue;

               int Tile = Collision()->GetCollisionAt(Col.x, Col.y);
               if(IsUnsafeHookTile(Tile))
                       continue;
